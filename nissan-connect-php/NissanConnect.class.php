...
class NissanConnect {
    ...
    public function startClimateControl($waitForResult = FALSE) {
        $this->prepare();
        $result = $this->sendRequest("hvac/vehicles/{$this->config->vin}/activateHVAC", ['executionTime' => date('c')]);
        return $result;
    }
    ...
    private function sendRequest($path, $params = array(), $method = 'POST') {
        ...
        if ($info['http_code'] !== 200) {
            if (($info['http_code'] == 401 || $info['http_code'] == 404) && $this->shouldRetry) {
                $this->debug("Request for '$path' failed. Response received: " . json_encode($result) . " Will retry.");
                $this->shouldRetry = FALSE; // Don't loop infinitely!
                $this->config->customSessionID = NULL;
                $this->prepare(TRUE);
                return $this->sendRequest($path, $params);
            }
            throw new Exception("Request for '$path' failed. Response received: " . json_encode($result), $info['http_code']);
        }
